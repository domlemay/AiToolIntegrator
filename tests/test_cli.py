...
class TestSearchCommand:
    ...
    def test_search_returns_table(self, mock_engine: MagicMock, tmp_path: Path) -> None:
    ...
    @patch("aitoolintegrator.cli.commands.search.Engine")
    def test_search_empty_query(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        mock_engine.return_value = _make_engine(tmp_path / "plugins")
        result = runner.invoke(app, ["search", ""])
        assert result.exit_code == 0

    @patch("aitoolintegrator.cli.commands.search.Engine")
    def test_search_with_tags(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        mock_engine.return_value = _make_engine(tmp_path / "plugins")
        result = runner.invoke(app, ["search", "test", "--tags", "test"])
        assert result.exit_code == 0

    @patch("aitoolintegrator.cli.commands.search.Engine")
    def test_search_sort_name(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        mock_engine.return_value = _make_engine(tmp_path / "plugins")
        result = runner.invoke(app, ["search", "", "--sort", "name"])
        assert result.exit_code == 0

    @patch("aitoolintegrator.cli.commands.search.Engine")
    def test_search_no_results_shows_warning(self, mock_engine: MagicMock, tmp_path: Path) -> None:
...
class TestListCommand:
    ...
    def test_list_all(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        mock_engine.return_value = _make_engine(tmp_path / "plugins")
    ...
    @patch("aitoolintegrator.cli.commands.list_tools.Engine")
    def test_list_installed_flag(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        mock_engine.return_value = _make_engine(tmp_path / "plugins")
        result = runner.invoke(app, ["list", "--installed"])
        assert result.exit_code == 0

    @patch("aitoolintegrator.cli.commands.list_tools.Engine")
    def test_list_empty_installed(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        engine = _make_engine(tmp_path / "plugins")
...
class TestInfoCommand:
    ...
    def test_info_existing_tool(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        mock_engine.return_value = _make_engine(tmp_path / "plugins")
    ...
    @patch("aitoolintegrator.cli.commands.info.Engine")
    def test_info_missing_tool(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        engine = _make_engine(tmp_path / "plugins")
...
class TestInstallCommand:
    ...
    def test_install_known_tool(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        mock_engine.return_value = _make_engine(tmp_path / "plugins")
        result = runner.invoke(app, ["install", "test-tool"])
        assert result.exit_code == 0

    @patch("aitoolintegrator.cli.commands.install.Engine")
    def test_install_unknown_tool(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        engine = _make_engine(tmp_path / "plugins")
        engine.get_tool_info.return_value = None
        mock_engine.return_value = engine
    ...
    @patch("aitoolintegrator.cli.commands.install.Engine")
    def test_install_failure_exits_1(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        engine = _make_engine(tmp_path / "plugins")
...
class TestUninstallCommand:
    ...
    @patch("aitoolintegrator.cli.commands.uninstall.Engine")
    def test_uninstall_not_installed(self, mock_engine: MagicMock, tmp_path: Path) -> None:
    ...
    @patch("aitoolintegrator.cli.commands.uninstall.Engine")
    def test_uninstall_prompts_without_yes(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        plugins = tmp_path / "plugins"
        plugins.mkdir()
        (plugins / "test-tool").mkdir()
        mock_engine.return_value = _make_engine(plugins)
...
class TestDoctorCommand:
    ...
    @patch("aitoolintegrator.cli.commands.doctor.Engine")
    def test_doctor_specific_tool(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        plugins = tmp_path / "plugins"
        plugins.mkdir()
        mock_engine.return_value = _make_engine(plugins)
    ...
    @patch("aitoolintegrator.cli.commands.doctor.Engine")
    def test_doctor_reports_issues(self, mock_engine: MagicMock, tmp_path: Path) -> None:
        plugins = tmp_path / "plugins"
        plugins.mkdir()
    ...
    @patch("aitoolintegrator.cli.commands.doctor.Engine")
    def test_doctor_no_plugins(self, mock_engine: MagicMock, tmp_path: Path) -> None:
